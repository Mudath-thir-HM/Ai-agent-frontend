...
const NODE_DEFAULTS: Record<string, { width: number; height: number }> = {
  ...
  prompt: { width: 260, height: 300 },
  output: { width: 280, height: 320 },
};
    ...
    <div className="text-xs text-white">
      <div className="flex items-center justify-between mb-2 px-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold text-[11px]">AgentCee_brand</span>
        </div>
      ...
      <div className="px-1 space-y-0.5">
        <p className="leading-relaxed text-[10px] text-zinc-300 line-clamp-3">
          <span className="font-semibold text-white">AgentCee_brand </span>
          {caption}
        </p>
  ...
  return (
    <div className="text-xs text-white">
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-1 mb-1">
            <span className="font-bold text-[11px]">AgentCee Brand</span>
            <span className="text-zinc-500 text-[10px]">
              @AgentCee_brand · now
            </span>
          </div>
          <p
            className={`text-[11px] leading-relaxed ${over ? "text-red-400" : "text-zinc-200"}`}
          >
            {trimmed}
          ...
          <div className="mt-2 border border-zinc-700 rounded-xl overflow-hidden">
            ...
            <div className="px-2 py-1 bg-zinc-900">
              <p className="text-[9px] text-zinc-400">AgentCee.ai</p>
              <p className="text-[10px] text-white font-medium">
                Start creating with AgentCee
              </p>
            </div>
          </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <span
          className={`text-[9px] ${over ? "text-red-400" : "text-zinc-500"}`}
        >
  ...
  return (
    <div className="text-xs text-white">
      <div className="flex items-center gap-2 mb-2">
        ...
        <div>
          <p className="font-semibold text-[11px]">AgentCee Brand</p>
          <p className="text-[9px] text-zinc-500">Just now · 🌍</p>
        </div>
      </div>
      <p className="text-[11px] text-zinc-200 leading-relaxed mb-2 line-clamp-4">
        {caption}
      </p>
      ...
      <div className="bg-zinc-800/50 rounded-lg p-3 space-y-2">
        ...
        <Row label="Size" className="mt-2">
          <span className="text-[11px] text-zinc-400">
            {Math.round((node.style?.width as number) ?? 220)} ×{" "}
            {Math.round((node.style?.height as number) ?? 260)}
          </span>
        </Row>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-zinc-500 mt-0.5">
              AI will match this image's aesthetic
            </p>
          </div>
          <button
          ...
          >
            <span
              className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${styleRef ? "translate-x-4" : "translate-x-0.5"}`}
            />
          </button>
        </div>
        {styleRef && (
          <div className="flex items-start gap-1.5 bg-[#1C82AD]/10 border border-[#1C82AD]/20 rounded px-2 py-1.5">
            ...
            <p className="text-[10px] text-[#1C82AD]">
              Style context passed to connected Prompt nodes
            </p>
          </div>
        )}
      </div>
...
function PromptNodePanel({
  ...
  const estimatedTokens =
    Math.ceil((node.data?.prompt?.length ?? 0) / 4) +
    connectedInputs.length * 512;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-800/50 rounded-lg p-3 space-y-2">
        ...
        {connectedInputs.length === 0 ? (
          <p className="text-[10px] text-zinc-600 italic">
            No nodes connected. Connect Image nodes to provide visual context.
          </p>
        ) : (
          <>
            {connectedInputs.map((n, i) => (
              ...
              >
                ...
                <span className="text-[10px] text-zinc-300 flex-1 truncate">
                  {n.data?.label ?? `Image ${i + 1}`}
                </span>
                <span className="text-[9px] text-[#03C988]">✓</span>
              </div>
            ))}
            <p className="text-[9px] text-zinc-500 mt-1">
              {connectedInputs.length} image
              {connectedInputs.length > 1 ? "s" : ""} will inform the AI
            </p>
          </>
        )}
      </div>
      <div className="bg-zinc-800/50 rounded-lg p-3 space-y-2">
        <Row label="Mode">
          <span
            className={`text-[11px] font-medium capitalize ${mode === "image" ? "text-[#1C82AD]" : "text-[#03C988]"}`}
          >
            {mode}
          </span>
        </Row>
        <Row label="Model" className="mt-1">
          <span className="text-[11px] text-zinc-400">
            {mode === "image" ? "FLUX.2 Klein 4B" : "DeepSeek Flash"}
          </span>
        </Row>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-[#1C82AD] font-mono">
      ...
      {!node.data?.prompt && (
        <p className="text-[10px] text-zinc-600 text-center">
          Add a prompt to the node first
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-1">
          {(generatedContent?.hashtags?.length
            ...
            <button
              key={tag}
              onClick={() =>
                setCaption((c) => (c.includes(tag) ? c : c + " " + tag))
              }
          ...
          <div className="bg-zinc-900 rounded-[14px] p-2.5 overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {platform === "facebook" && (
                  <FacebookPreview caption={caption} />
                )}
              </motion.div>
            </AnimatePresence>
          </div>
      ...
      {generatedContent && (
        <div className="bg-[#03C988]/10 border border-[#03C988]/30 rounded-lg px-3 py-2">
          <p className="text-[10px] text-[#03C988] font-medium flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3" /> Content generated — ready to
            schedule
          </p>
        </div>
      )}
      ...
      >
        ...
        {scheduleTime && (
          <span className="text-zinc-400">@ {scheduleTime}</span>
        )}
      </button>
...
export function CanvasOverlay({ isOpen, onClose }: CanvasOverlayProps) {
  ...
  const handleGenerate = useCallback(
    async (prompt: string, mode: string, creativity: number) => {
      if (!prompt.trim()) return;
      const toneMap = (c: number) =>
        c < 0.35 ? "professional" : c < 0.7 ? "casual" : "humorous";
      try {
        ...
        toast.error(
          err?.data?.detail ?? "Generation failed. Please try again.",
        );
      }
    },
    [generateContent],
  ...
  const addNode = (type: string) => {
    ...
    if (type === "image") {
      ...
      newNode = {
        ...
        position,
        style: { ...defaults },
        data: { label: "AI Prompt", prompt: "", mode: "caption" },
      };
    } else if (type === "output") {
      newNode = {
        id: `output-${Date.now()}`,
        ...
        position,
        style: { ...defaults },
        data: { label: "Output Card", mode: "caption" },
      };
    } else {
      newNode = {
        id: `text-${Date.now()}`,
            ...
            >
              ...
              {toolbarItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute inset-0 rounded-xl bg-gradient-to-br ${item.color} opacity-0 group-hover:opacity-20 transition-opacity`}
                    />
                    <Icon className="w-5 h-5 text-zinc-400 group-hover:text-white transition-colors" />
                    <span className="text-[9px] text-zinc-600 group-hover:text-zinc-400 transition-colors leading-none">
                      {item.label}
                    </span>
              ...
              <div className="mt-auto px-2 pb-2">
                <p className="text-[9px] text-zinc-700 text-center leading-tight">
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-zinc-600 text-sm max-w-xs">
                      Add nodes from the sidebar. Connect multiple Image nodes
                      into one AI Prompt for richer results.
                    </p>
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-4">
                    {selectedNode ? (
                      <>
                        {selectedNode.type === "imageNode" && (
                          <ImageNodePanel node={selectedNode} />
                        )}
                        {selectedNode.type === "promptNode" && (
                          <PromptNodePanel
                            node={selectedNode}
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-center gap-3 py-10">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-zinc-600 max-w-[180px]">
                            Click any node on the canvas to see context-aware
                            controls
                          </p>
                        </div>
                          ...
                          {[
                            {
                              color: "#1C82AD",
                              label: "Image node",
                              desc: "Upload + style ref",
                            },
                            {
                              color: "#7c3aed",
                              label: "Prompt node",
                              desc: "Context + creativity",
                            },
                            {
                              color: "#03C988",
                              label: "Output node",
                              desc: "Preview + schedule",
                            },
                          ].map((h) => (
                            <div
                              key={h.label}
